...
""",
    ...
    def add_variables(self):
        ...
        # add components
        self.solutes = Set(initialize=["Li", "Al", "Cl"])
        self.cations = Set(initialize=["Li", "Al"])
        ...
        def initialize_diffusion_params(m, i, j, k):
            if self.config.charged_membrane:
                # these params assume chi=-140 mM and lithium mem_conc range 50-80 mM & aluminum mem_conc range 2-32 mM
                vals = {
                    "Li": {
                        "Li": {0: -9.39e-06, 1: 1.07e-07, 2: -1.49e-07},
                        "Al": {0: -2.51e-05, 1: 4.70e-07, 2: -6.58e-07},
                    },
                    "Al": {
                        "Li": {0: -3.65e-06, 1: 4.80e-08, 2: -2.25e-08},
                        "Al": {0: -1.81e-05, 1: 2.12e-07, 2: -9.94e-08},
                    },
                }
            else:
                # these params assume chi=0 mM and lithium mem_conc range 50-80 mM & aluminum mem_conc range 2-32 mM
                vals = {
                    "Li": {
                        "Li": {0: -4.43e-06, 1: -4.21e-09, 2: 2.02e-08},
                        "Al": {0: -3.24e-06, 1: -1.86e-08, 2: 8.91e-08},
                    },
                    "Al": {
                        "Li": {0: -2.14e-07, 1: 1.89e-09, 2: -9.06e-09},
                        "Al": {0: -2.95e-06, 1: 8.32e-09, 2: -3.99e-08},
                    },
                }
            return vals[i][j][k]
        ...
        def initialize_convection_params(m, j, k):
            if self.config.charged_membrane:
                # these params assume chi=-140 mM and lithium mem_conc range 50-80 mM & aluminum mem_conc range 80-110 mM
                vals = {
                    "Li": {0: -4.28, 1: 0.0895, 2: -0.0835},
                    "Al": {0: -7.61, 1: 0.146, 2: -0.136},
                }
            else:
                # these params assume chi=0 mM and lithium & aluminum mem_conc range 50-80 mM
                vals = {"Li": {0: 1, 1: 0, 2: 0}, "Al": {0: 1, 1: 0, 2: 0}}
            return vals[j][k]
    ...
    def add_scaling_factors(self):
        ...
        for x in self.dimensionless_module_length:
            if x != 0:
                ...
                self.scaling_factor[self.mol_flux_aluminum[x]] = 1e-2
            ...
            for z in self.dimensionless_membrane_thickness:
                ...
                self.scaling_factor[self.membrane_conc_mol_lithium[x, z]] = 1e-3
                self.scaling_factor[self.membrane_conc_mol_aluminum[x, z]] = 1e-2
                self.scaling_factor[self.membrane_conc_mol_chloride[x, z]] = 1e-3
