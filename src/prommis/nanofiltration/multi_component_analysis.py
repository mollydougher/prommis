...
def build_model(
    ...
    elif cation_list[0] == "Na":
        m.fs.membrane.applied_pressure.fix(data_applied_pressure)
    elif cation_list[0] == "Ca":
        m.fs.membrane.applied_pressure.fix(data_applied_pressure)
    elif cation_list[0] == "La":
        m.fs.membrane.applied_pressure.fix(data_applied_pressure)
    else:
...
def plot_flux_contributions(x_axis="ionic_strength", percent=False, save_figure=True):
    ...
    cl_lithium_bl_df.plot(
        ax=ax4a, kind="bar", stacked=True, color=color_list, rot=0, legend=False
    )
    # cobalt_bl_df.plot(
