...
class MultiComponentDiafiltrationInitializer(BlockTriangularizationInitializer):
    ...
    def initialization_routine(self, model):
        ...
        D_bl_bilinear = model.boundary_layer_cross_diffusion_coefficient_bilinear
        D_bl_bilinear_calc = (
        ...
        D_mem_bilinear = model.membrane_cross_diffusion_coefficient_bilinear
        D_mem_bilinear_calc = (
            ...
            for x in model.dimensionless_module_length:
                if x != 0:
                    ...
                    for z_m in model.dimensionless_membrane_thickness:
                        ...
                        for k in model.cations:
                            calculate_variable_from_constraint(
                                alpha_mem_bilinear[t, x, z_m, k],
                                alpha_mem_calc[t, x, z_m, k],
                            )
                            calculate_variable_from_constraint(
                                alpha_mem[t, x, z_m, k],
                                alpha_mem_bilinear_calc[t, x, z_m, k],
                            )
                            for j in model.cations:
                                calculate_variable_from_constraint(
                                    D_mem_bilinear[t, x, z_m, k, j],
                                    D_mem_calc[t, x, z_m, k, j],
                                )
                                calculate_variable_from_constraint(
                                    D_mem[t, x, z_m, k, j],
                                    D_mem_bilinear_calc[t, x, z_m, k, j],
                                )
                        z_m_prev = z_m
                x_prev = x
...
""",
    ...
    def add_constraints(self):
        ...
        def _membrane_convection_coefficient_bilinear_calculation(blk, t, x, z, j):
            if x == 0:
                return Constraint.Skip
            return (
                blk.membrane_convection_coefficient_bilinear[t, x, z, j]
                == blk.membrane_convection_coefficient[t, x, z, j]
                * blk.membrane_D_tilde[t, x, z]
            )
        ...
        def _membrane_convection_coefficient_calculation(blk, t, x, z, j):
            if x == 0:
                return Constraint.Skip
            charge = blk.config.property_package.charge
            chi = blk.membrane_fixed_charge
            D_mem = blk.config.property_package.membrane_diffusion_coefficient
            return blk.membrane_convection_coefficient_bilinear[t, x, z, j] == (
                blk.membrane_D_tilde[t, x, z] + (charge[j] * D_mem[j] * chi)
            )
